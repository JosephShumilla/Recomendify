...
5. If the UI shows "Error occurred while reading given playlist," check the following:
   * Your Spotify credentials are exported in the same terminal session where you run `netlify dev`.
   * The playlist URL is public (or shared with the credentialed account) and matches the format `https://open.spotify.com/playlist/<id>`.
   * Your account has access to `playlist-read-private` and the playlist contains tracks with available audio features.
